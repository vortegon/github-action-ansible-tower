...
async function launchJob(requestData: any) {
  console.log(
    `Launching Template ID ${requestData.templateId} on Ansible Tower...`
  );
  ...
  if (response && response.job) {
    ...
    console.log(
      `Job ${response.job} was created on Ansible Tower: Status ${response.status}.`
    );
    return response.url;
    ...
    throw new Error(
      `Template ID ${requestData.templateId} couldn't be launched, the Ansible API is not working`
    );
  }
}
...
async function printAnsibleOutput(requestData: any, jobData: any) {
  ...
  if (jobData.status == "failed" && response) {
    ...
    console.log(response);
    console.log(
      "***************************Ansible Tower traceback output***************************"
    );
    console.log(jobData.result_traceback);
    throw new Error(
      `An error has ocurred on Ansible tower trying to launch job ${jobData.id}`
    );
    ...
    console.log(`Final status: ${jobData.status}`);
    console.log(
      "[warning]: An error ocurred trying to get the ansible tower output"
    );
    console.log(response);
  }
  return response;
